...
#library for SAM2 model (segment anything) 
#there's actually grounding dino + SAM for text prompt based segmentation, but too big to handle
...
def remove_background_SAM2(input_path):
	print(f"Removing background using sam2 for image: {input_path}")
	model = SAM("sam2_b.pt") #change to sam2_l.pt for maximum accuracy but big computation
	result = model(input_path)
	
	#creating mask template from whole detection
	input_img = np.array(Image.open(input_path), dtype=np.uint8)
	w, h = Image.open(input_path).size
	new_img = np.zeros((h,w))
	masks = []
	
	#appending mask for every detection
	for i in result[0]:
		masks.append(i.masks.data[0].tolist())
	#assigning detection masks to mask template
	for i in masks:
		new_img = np.maximum(new_img, i)
	img_mask = np.array(new_img*255, dtype=np.uint8)
	#bitwise_and within 4 channels
	masked_img = np.bitwise_and(input_img[:, :, :4], img_mask[:, :, None])
...
def process_single_image(image_path, output_folder, bg_method, canvas_size_name, output_format, bg_choice, custom_color, watermark_path=None):
	...
	try:
		...
		#add condition for sam2 usage
		elif bg_method == 'sam2':
			image_with_no_bg = remove_background_SAM2(image_path)
		
		#add condition for no background removal method
		elif bg_method == 'none':
		...
		#instead checking the filename, just check if watermark path is provided or not
		if watermark_path:
			watermark = Image.open(watermark_path).convert("RGBA")
			#resizing watermark image to be same like the processed image
			watermark = watermark.resize(canvas.size, Image.LANCZOS)
...
with gr.Blocks(theme="NoCrypt/miku@1.2.2") as iface:
	...
	with gr.Row():
		...
		watermark = gr.File(label="Upload Watermark Image (Optional)", file_types=[".png"], interactive=True)
	...
	with gr.Row():
		#add 'none' and 'SAM2' choice for bg_method
		bg_method = gr.Radio(choices=["bria", "rembg", "sam2", "none"], label="Background Removal Method", value="bria")
	...
	#add function to display prompt box
